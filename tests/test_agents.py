...
def test_agent_execute_task_insufficient_info(sample_dna):
    """Test task execution with insufficient information"""
    agent = Agent(dna=sample_dna)
    
    # Very vague task should trigger clarification
...
def test_agent_execute_task_with_context(sample_dna):
    """Test task execution with proper context"""
    agent = Agent(dna=sample_dna)
    ...
    # Should execute (or at least not require clarification)
    assert result["status"] in ["completed", "clarification_needed"]
